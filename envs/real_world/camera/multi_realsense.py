...
from envs.real_world.camera.single_realsense import SingleRealsense
# from real_world.camera.single_femto import SingleFemto
from envs.real_world.camera.video_recorder import VideoRecorder
...
class MultiRealsense:
    def __init__(self,
        ...
        # if femto_serial_numbers is None:
        #     femto_serial_numbers = SingleFemto.get_connected_devices_serial()
        # n_femto_cameras = len(femto_serial_numbers)
        ...
        # for i, serial in enumerate(femto_serial_numbers):
        #     femto_cameras[serial] = SingleFemto(
        #         shm_manager=shm_manager,
        #         serial_number=serial,
        ...
        #         enable_infrared=False,
        #         get_max_k=get_max_k,
        #         advanced_mode_config=advanced_mode_config[i+n_cameras],
        #         transform=transform[i+n_cameras],
        #         vis_transform=vis_transform[i+n_cameras],
        #         recording_transform=recording_transform[i+n_cameras],
        #         video_recorder=video_recorder[i+n_cameras],
        #         verbose=False
        #     )
        
        self.cameras = cameras
        self.serial_numbers = serial_numbers
        # self.femto_cameras = femto_cameras
        # self.femto_serial_numbers = femto_serial_numbers
    ...
    # @property
    # def n_femto_cameras(self):
    #     return len(self.femto_cameras)
    ...
    def is_ready(self):
        ...
        for camera in self.cameras.values():
            if not camera.is_ready:
                is_ready = False
        # for camera in self.femto_cameras.values():
        #     if not camera.is_ready:
        #         is_ready = False
        return is_ready
    ...
    def get(self, k=None, index=None, out=None) -> Dict[int, Dict[str, np.ndarray]]:
        ...
        for i, camera in enumerate(self.cameras.values()):
            ...
            out[i] = this_out
        # for i, camera in enumerate(self.femto_cameras.values()):
        #     this_out = None
        #     if f'femto_{i}' in out:
        #         this_out = out[f'femto_{i}']
        #     this_out = camera.get(k=k, out=this_out)
        #     out[f'femto_{i}'] = this_out
        return out
    ...
    # def set_femto_exposure(self, exposure=None, gain=None):
    #     for camera in self.femto_cameras.values():
    ...
    # def set_femto_white_balance(self, white_balance=None):
    #     for camera in self.femto_cameras.values():
    #         camera.set_white_balance(white_balance=white_balance)
    
    def get_intrinsics(self):
        return np.array([c.get_intrinsics() for c in self.cameras.values()])
    
    # def get_femto_intrinsics(self):
    #     return np.array([c.get_intrinsics() for c in self.femto_cameras.values()])
    
    def get_depth_scale(self):
        return np.array([c.get_depth_scale() for c in self.cameras.values()])
    
    # def get_femto_depth_scale(self):
    #     return np.array([c.get_depth_scale() for c in self.femto_cameras.values()])
    ...
    def start_recording(self, video_path: Union[str, List[str]], start_time: float):
        if isinstance(video_path, str):
            ...
            # femto_video_path = list()
            # for i in range(self.n_femto_cameras):
            #     femto_video_path.append(
            #         str(video_dir.joinpath(f'femto_{i}.mp4').absolute()))
        # assert len(video_path) == self.n_cameras
        # assert len(femto_video_path) == self.n_femto_cameras

        # for i, camera in enumerate(self.cameras.values()):
        #     camera.start_recording(video_path[i], start_time)
        # for i, camera in enumerate(self.femto_cameras.values()):  # only femto
        #     camera.start_recording(femto_video_path[i], start_time)
