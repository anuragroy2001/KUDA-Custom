...
class XARM6:
    ...
    def move_to_pose(self, pose, wait=True, ignore_error=False):
        ...
        code = self._arm.set_position(
            pose[0], pose[1], pose[2], pose[3], pose[4], pose[5], speed=100, wait=wait
        )
        if not ignore_error:
    ...
    def reset(self):
        self.move_to_pose(self.init_pose, wait=True)
        # self._arm.set_servo_angle(angle=self.init_servo_angle, isradian=False)
